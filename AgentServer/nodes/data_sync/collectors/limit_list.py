...
class LimitListCollector(BaseCollector):
    ...
    async def collect(self) -> Dict[str, Any]:
        ...
        self.logger.info(
            f"Syncing limit list data: {start_date} -> {end_date}"
        )
        ...
        while current_date <= end_dt:
            ...
            try:
                t1 = time.time()
                records = await tushare_manager.get_limit_list_d(
                ...
                if records:
                    ...
                    self.logger.info(
                        f"  {trade_date}: {len(records)} stocks, API={t2-t1:.2f}s"
                    )
        ...
        return {
            ...
            "end_date": end_date,
            "message": f"Synced {total_count} limit list records from {start_date} to {end_date}",
        }
