...
class MoneyflowConceptCollector(BaseCollector):
    ...
    default_schedule = "5 16 * * 1-5"  # 默认: 每个交易日 16:05
    ...
    async def collect(self) -> Dict[str, Any]:
        ...
        self.logger.info(
            f"Syncing moneyflow concept data: {start_date} -> {end_date}"
        )
        ...
        while current_date <= end_dt:
            ...
            try:
                t1 = time.time()
                records = await tushare_manager.get_moneyflow_cnt_ths(
                ...
                if records:
                    ...
                    self.logger.info(
                        f"  {trade_date}: {len(records)} concepts, API={t2-t1:.2f}s"
                    )
        ...
        return {
            ...
            "end_date": end_date,
            "message": f"Synced {total_count} moneyflow concept records from {start_date} to {end_date}",
        }
